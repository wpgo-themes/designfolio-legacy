...
class PC_Framework {
	...
	public function setup_locale() {
		...
		if ( file_exists( get_stylesheet_directory() . '/' . $locale_filename ) ) {
			load_theme_textdomain( 'presscoders', get_stylesheet_directory() . '/' . $locale_mofile_dir );
		} else {
			if ( file_exists( get_template_directory() . '/' . $locale_filename ) ) {
				load_theme_textdomain( 'presscoders', get_template_directory() . '/' . $locale_mofile_dir );
			}
		}
	}
	...
	public function framework_features() {
		...
		if ( current_theme_supports( 'theme-options-page' ) ) {
			...
			if ( current_theme_supports( 'homepage-bar' ) ) {
				...
				/* Use specified hook to render before content widget area. */
				if ( is_array( $_wp_theme_features['homepage-bar'] ) ) {
					$homepage_bar_hook     = $_wp_theme_features['homepage-bar'][0];
					$homepage_bar_priority = isset( $_wp_theme_features['homepage-bar'][1] ) ? $_wp_theme_features['homepage-bar'][1] : 10;
					add_action( $homepage_bar_hook, array( &$this->_utility_callbacks_class, 'render_homepage_bar' ), $homepage_bar_priority );
				} else {
		...
		if ( current_theme_supports( 'custom-post-types' ) ) {
			...
			if ( is_array( $_wp_theme_features['custom-post-types'] ) ) {
				...
				if ( is_array( $custom_post_types ) ) {
					...
					if ( in_array( 'testimonials', $custom_post_types ) && class_exists( 'PC_Testimonial_CPT' ) ) {
						$this->_testimonials_cpt_class = new PC_Testimonial_CPT();
					}

					/* Add Slides CTP if specified. */
					if ( in_array( 'slides', $custom_post_types ) && class_exists( 'PC_Slide_CPT' ) ) {
						$this->_slides_cpt_class = new PC_Slide_CPT();
					}
					...
					if ( in_array( 'support', $custom_post_types ) && class_exists( 'PC_Support_CPT' ) ) {
						$this->_support_cpt_class = new PC_Support_CPT();
					}
				}
			}
		}
	...
	public function enqueue_modernizr() {
		...
		if ( ! is_admin() ) {
			...
			//wp_register_script( 'pc_modernizr', PC_THEME_ROOT_URI.'/api/js/html5/modernizr/modernizr.custom.js' );
	...
	public function theme_activated() {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
					...
					if ( PC_INSTALL_CONTENT_PROMPT ) {
						...
						<div class="updated" style="margin-top: 10px;padding-bottom:10px;">
							...
							<ul>
								...
								<li>
									<strong>Navigation Menu</strong> - Pages added to the main navigation menu, along with a home page link.
								</li>
							</ul>
			...
			?>
			<div class="updated">
			<p><?php printf( PC_THEME_NAME . ' theme activated! <a href="%s">Visit Site</a>', home_url() ); ?></p>
			</div><?php
		}
	}
	...
	public function theme_redirect_cb() {
		...
		// @todo see how this compares to the global $pc_template variable set earlier.
		if ( ! is_archive() && ! is_404() && ! is_search() && ! is_feed() ) {
