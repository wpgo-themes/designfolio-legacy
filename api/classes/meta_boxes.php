...
class PC_MetaBoxes {
		...
		<div id="page-sidebar-commander">
			<div class="tabs">
				...
				if ( $layout == "1-col" && $pc_page_template != "widgetized-page.php" ) {
					...
					<ul>
						<?php if ( $layout != "1-col" ) { ?>
							<li><a href="#tabs-primary"><?php _e( 'Primary Sidebar', 'presscoders' ); ?></a>
							</li> <?php } ?>
						<?php
				...
				<?php if ( $layout != "1-col" ) {
					...
					<div id="tabs-primary">
						...
						<p class="column_message">
							<img class="layouticon" src="<?php echo $primary_layout_icon; ?>" /><?php echo $msg; ?></p>
					</div><!-- #tabs-primary -->
				...
				if ( $pc_page_template == "widgetized-page.php" ) {
					...
					<div id="tabs-maincontent">
						...
						<p class="column_message">
							<img class="layouticon" src="<?php echo $main_content_layout_icon; ?>" /><?php echo $msg; ?>
						</p>
				...
				<?php if ( $layout == "3-col-l" || $layout == "3-col-r" || $layout == "3-col-c" ) {
					...
					<div id="tabs-secondary">
						...
						<p class="column_message">
							<img class="layouticon" src="<?php echo $secondary_layout_icon; ?>" /><?php echo $msg; ?>
						</p>
					</div><!-- #tabs-secondary -->
				<?php } ?>
			</div>
	...
	public function editor_jquery_ui_tabs() {
		...
		wp_enqueue_style( 'theme_admin_stylesheet', PC_THEME_ROOT_URI . '/api/css/theme_admin.css' );
	...
	public function theme_meta_box_display_options( $post, $args ) {
		...
		if ( $page_type == 'page' ) {
			...
			<div class="inside"><p>
					<label class="selectit"><input id="hide_title_header_tag" name="<?php echo PC_THEME_NAME_SLUG; ?>_hide_title_header_tag" type="checkbox" value="1" class="alignleft" <?php if ( isset( $hide_title_header_tag ) ) {
							checked( '1', $hide_title_header_tag );
						} ?> />&nbsp;Hide <?php echo $page_type; ?> title</label></p></div>
			...
			if ( post_type_exists( 'portfolio' ) && isset( $post->page_template ) && $post->page_template == 'portfolio-page.php' && ! empty( $pf_group_exists ) ) {
				...
				<div class="inside" style="margin-bottom:1px;">
					<label for="<?php echo $pc_portfolio_group; ?>"><?php _e( 'Portfolio group', 'presscoders' ); ?>:</label>
	...
	public function theme_meta_box_seo( $post, $box ) {
		...
		<table class="form-table" id="seosettings">
			...
			<tr>
				...
				<td>
					<input type="text" name="<?php echo PC_THEME_NAME_SLUG; ?>_title" id="pc_theme_title" value="<?php echo esc_attr( $pc_theme_title ); ?>" />
				</td>
			</tr>
			<tr>
				<th style="width: 10%;">
					<label for="Description"><?php _e( 'Content Description', 'presscoders' ); ?>:</label></th>
				<td>
					<textarea rows="4" name="<?php echo PC_THEME_NAME_SLUG; ?>_description" id="pc_theme_description"><?php echo esc_attr( $pc_theme_description ); ?></textarea>
				</td>
			</tr>
			<tr>
				<th style="width: 10%;"><label for="Keywords"><?php _e( 'SEO Keywords', 'presscoders' ); ?>:</label>
				</th>
				<td>
