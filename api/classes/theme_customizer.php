...
class PC_Theme_Customizer {
	...
	public function __construct() {
		...
		add_action( 'customize_controls_enqueue_scripts', array( &$this, 'enqueue_theme_specific_customizer_js' ) );
	}
	...
	public function initialize_customizer_defaults() {
		...
		$pc_customizer_defaults = array( 'chk_hide_description'        => null,
										 ...
										 'pc-google-webfonts-selector' => 'h1, h2, h3, h4, nav a'
		);
	}
	...
	public function alter_title_and_tagline_label() {
		...
		<script type="text/javascript" defer="defer">
			(function ($) {
				jQuery('li#customize-section-title_tagline h3', window.top.document).text('Site Logo, Title & Tagline', '#00ff00');
			})(jQuery)
		</script>
	<?php
	}
	...
	public function enqueue_customizer_google_font() {
		...
		if ( empty( $pc_google_custom_webfont ) ) {
			...
			$theme_google_font = str_replace( 'https://fonts.googleapis.com/css?family=', '', $pc_google_custom_webfont );
		}
	...
	/**
	 * Enqueue custom JS/jQuery to the theme customizer header.
	 *
	 * @since 0.1.0
	 */
	public function enqueue_theme_specific_customizer_js() {
		$res = file_exists( PC_THEME_ROOT_DIR . '/includes/js/pc-theme-specific-customizer.js' );
		if ( file_exists( PC_THEME_ROOT_DIR . '/includes/js/pc-theme-specific-customizer.js' ) ) {
			wp_register_script( 'pc-theme-specific-customizer-js', PC_THEME_ROOT_URI . '/includes/js/pc-theme-specific-customizer.js' );
			wp_enqueue_script( 'pc-theme-specific-customizer-js' );
		}
	}
}
