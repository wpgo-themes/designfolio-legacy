...
class PC_Theme_Options {
	...
	public function theme_header_html() {
		...
		if ( ! empty( $options['txtarea_header'] ) ) {
			echo $options['txtarea_header'];
		}
	}
	...
	public function theme_footer_html() {
		$options = get_option( PC_OPTIONS_DB_NAME );
		if ( ! empty( $options['txtarea_footer'] ) ) {
			echo $options['txtarea_footer'];
		}
	}
	...
	public function theme_admin_init() {
		...
		// Register admin theme option style sheet
		wp_register_style( 'theme_admin_stylesheet', PC_THEME_ROOT_URI . '/api/css/theme_admin.css' );
