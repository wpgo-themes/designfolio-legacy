...
class PC_Utility {
	...
	public static function content_layout_classes_primary( $sidebar = false ) {
		...
		if ( $sidebar == true ) { /* Add classes to sidebar. */
			if ( $cols == "1-col" ) {
				...
				return "left sidebar-extra-margin-right";
			} elseif ( $cols == "3-col-r" ) {
				return "right sidebar-extra-margin-right";
			} elseif ( $cols == "3-col-c" ) {
				return "left sidebar-extra-margin-right";
			}
	...
	public static function pc_display_theme_logo( $options ) {
		...
		if ( current_theme_supports( 'custom-logo' ) && isset( $options[PC_LOGO_CHK_OPTION_NAME] ) && $options[PC_LOGO_CHK_OPTION_NAME] ) {
			...
			if ( $options[PC_LOGO_URL_OPTION_NAME] != "" ) {
				?>
				<div id="site-logo"><a href="<?php echo get_home_url(); ?>"><img src="<?php echo $logo_url; ?>" /></a>
				</div>
			<?php
			} else {
	 ...
	 * Show logo in site header.
	 *
	 * This function replaces pc_display_theme_logo() to display the site logo. It uses the theme customizer to handle the site logo.
	 *
	 * @since 0.1.0
	 */
	public static function pc_display_customizer_site_logo() {
		...
		if ( ! empty( $pc_txt_logo_url ) ) {
			?>
			<div id="site-logo">
				<a href="<?php echo get_home_url(); ?>" /><img src="<?php echo $pc_txt_logo_url; ?>" /></a></div>
		<?php
		} // Otherwise show title text instead of logo
		else {
			if ( is_front_page() ) {
	...
	public static function install_default_content( $theme_options_url ) {
		...
		$info_box_widget = array(
			...
			'default_settings' => array(
				...
				'flickr_id'        => '',
				'googleplus_id'    => 'https://plus.google.com',
				'linkedin_id'      => 'https://www.linkedin.com',
				'rss_id'           => 'https://www.presscoders.com',
				'custom_id_1'      => '',
				'custom_img_1'     => '',
				'show_search'      => ''
			),
	...
	public static function render_widget_area( $widget_area_name = '', $show_default = false, $default_file = '', $check_global = false ) {
		...
		if ( $show_default && empty( $default_file ) ) {
			$default_file = 'primary_post_generic_default_widgets.php';
		}
		...
		if ( is_active_sidebar( $widget_area_name ) ) :
			...
			if ( $show_default ) {
				...
				if ( $check_global ) {
					...
					if ( ! is_active_sidebar( 'global-widget-area' ) ) {
