	...
	public function add_widget_callback( $value, $key ) {
		switch ( $value ) {
			...
			case 'nivo-slider':
				if ( file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-nivo-slider.php' ) ) {
					...
					if ( ! defined( 'PC_SLIDER_IMG_THIRD_WIDTH' ) ) {
						define( 'PC_SLIDER_IMG_THIRD_WIDTH', 300 );
					}
					if ( ! defined( 'PC_SLIDER_IMG_TWO_THIRDS_WIDTH' ) ) {
						define( 'PC_SLIDER_IMG_TWO_THIRDS_WIDTH', 640 );
					}
					if ( ! defined( 'PC_SLIDER_IMG_FULL_WIDTH' ) ) {
						define( 'PC_SLIDER_IMG_FULL_WIDTH', 940 );
					}
					if ( ! defined( 'PC_SLIDER_IMG_HEIGHT' ) ) {
						define( 'PC_SLIDER_IMG_HEIGHT', 300 );
					}
					if ( ! defined( 'PC_SLIDER_NAV_ARROWS' ) ) {
						define( 'PC_SLIDER_NAV_ARROWS', false );
					}
			...
			case 'content-slider':
				...
				if ( file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-content-slider.php' ) ) {
					...
					if ( ! defined( 'PC_SLIDER_CONTENT_IMG_THIRD_WIDTH' ) ) {
						define( 'PC_SLIDER_CONTENT_IMG_THIRD_WIDTH', 300 );
					}
					if ( ! defined( 'PC_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH' ) ) {
						define( 'PC_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH', 640 );
					}
					if ( ! defined( 'PC_SLIDER_CONTENT_IMG_FULL_WIDTH' ) ) {
						define( 'PC_SLIDER_CONTENT_IMG_FULL_WIDTH', 940 );
					}
					if ( ! defined( 'PC_SLIDER_CONTENT_IMG_HEIGHT' ) ) {
						define( 'PC_SLIDER_CONTENT_IMG_HEIGHT', 300 );
					}
					if ( ! defined( 'PC_SLIDER_CONTENT_NAV_ARROWS' ) ) {
						define( 'PC_SLIDER_CONTENT_NAV_ARROWS', false );
					}
			...
			case 'portfolio-slider':
				...
				if ( file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-portfolio-cpt-slider.php' ) ) {
					...
					/* Use default content slider sizes, with a fixed height. */
					if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_THIRD_WIDTH' ) ) {
						define( 'PC_PF_SLIDER_CONTENT_IMG_THIRD_WIDTH', 300 );
					}
					if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH' ) ) {
						define( 'PC_PF_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH', 640 );
					}
					if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_FULL_WIDTH' ) ) {
						define( 'PC_PF_SLIDER_CONTENT_IMG_FULL_WIDTH', 940 );
					}
					if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_HEIGHT' ) ) {
						define( 'PC_PF_SLIDER_CONTENT_IMG_HEIGHT', 300 );
					}
					if ( ! defined( 'PC_PF_SLIDER_CONTENT_NAV_ARROWS' ) ) {
						define( 'PC_PF_SLIDER_CONTENT_NAV_ARROWS', false );
					}
			...
			case 'carousel-slider':
				...
				if ( file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-carousel-slider.php' ) ) {
					...
					/* Use default slider sizes. */
					if ( ! defined( 'PC_CAROUSEL_CONTENT_WIDTH' ) ) {
						define( 'PC_CAROUSEL_CONTENT_WIDTH', 100 );
					}
					if ( ! defined( 'PC_CAROUSEL_CONTENT_HEIGHT' ) ) {
						define( 'PC_CAROUSEL_CONTENT_HEIGHT', 100 );
					}
					if ( ! defined( 'PC_CAROUSEL_CONTENT_NAV_ARROWS' ) ) {
						define( 'PC_CAROUSEL_CONTENT_NAV_ARROWS', true );
					}
		...
		if ( is_array( $value ) ) {
			...
			if ( array_key_exists( 'nivo-slider', $value ) && file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-nivo-slider.php' ) ) {
				...
				if ( ! defined( 'PC_SLIDER_IMG_THIRD_WIDTH' ) ) {
					define( 'PC_SLIDER_IMG_THIRD_WIDTH', $one_third_width );
				}
				if ( ! defined( 'PC_SLIDER_IMG_TWO_THIRDS_WIDTH' ) ) {
					define( 'PC_SLIDER_IMG_TWO_THIRDS_WIDTH', $two_thirds_width );
				}
				if ( ! defined( 'PC_SLIDER_IMG_FULL_WIDTH' ) ) {
					define( 'PC_SLIDER_IMG_FULL_WIDTH', $full_width );
				}
				if ( ! defined( 'PC_SLIDER_IMG_HEIGHT' ) ) {
					define( 'PC_SLIDER_IMG_HEIGHT', $height );
				}
				if ( ! defined( 'PC_SLIDER_NAV_ARROWS' ) ) {
					define( 'PC_SLIDER_NAV_ARROWS', $nav_arrows );
				}
			...
			if ( array_key_exists( 'content-slider', $value ) && file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-content-slider.php' ) ) {
				...
				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_THIRD_WIDTH' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_THIRD_WIDTH', $one_third_width );
				}
				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH', $two_thirds_width );
				}
				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_FULL_WIDTH' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_FULL_WIDTH', $full_width );
				}
				if ( ! defined( 'PC_SLIDER_CONTENT_NAV_ARROWS' ) ) {
					define( 'PC_SLIDER_CONTENT_NAV_ARROWS', $nav_arrows );
				}

				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_ONE_THIRD_HEIGHT' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_ONE_THIRD_HEIGHT', $one_third_height );
				}
				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_TWO_THIRDS_HEIGHT' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_TWO_THIRDS_HEIGHT', $two_thirds_height );
				}
				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_FULL_HEIGHT' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_FULL_HEIGHT', $full_third_height );
				}
				if ( ! defined( 'PC_SLIDER_CONTENT_IMG_HEIGHT' ) ) {
					define( 'PC_SLIDER_CONTENT_IMG_HEIGHT', $height );
				}
			...
			if ( array_key_exists( 'portfolio-slider', $value ) && file_exists( PC_THEME_ROOT_DIR . '/api/classes/widgets/widget-portfolio-cpt-slider.php' ) ) {
				...
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_THIRD_WIDTH' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_THIRD_WIDTH', $one_third_width );
				}
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_TWO_THIRDS_WIDTH', $two_thirds_width );
				}
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_FULL_WIDTH' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_FULL_WIDTH', $full_width );
				}
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_NAV_ARROWS' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_NAV_ARROWS', $nav_arrows );
				}

				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_ONE_THIRD_HEIGHT' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_ONE_THIRD_HEIGHT', $one_third_height );
				}
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_TWO_THIRDS_HEIGHT' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_TWO_THIRDS_HEIGHT', $two_thirds_height );
				}
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_FULL_HEIGHT' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_FULL_HEIGHT', $full_third_height );
				}
				if ( ! defined( 'PC_PF_SLIDER_CONTENT_IMG_HEIGHT' ) ) {
					define( 'PC_PF_SLIDER_CONTENT_IMG_HEIGHT', $height );
				}
