...
class pc_news_ticker_widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'pc_news_ticker_widget_' . PC_THEME_NAME_SLUG, 'News Ticker', $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'presscoders' ); ?></label>&nbsp;&nbsp;
			<small style="font-style:italic;color:#777;">(max 13 chars)</small>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number_posts' ); ?>"><?php _e( 'Number of posts to show:', 'presscoders' ); ?></label>&nbsp;&nbsp;
			<input id="<?php echo $this->get_field_id( 'number_posts' ); ?>" name="<?php echo $this->get_field_name( 'number_posts' ); ?>" type="text" value="<?php echo $number_posts; ?>" size="4" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'pause_time' ); ?>"><?php _e( 'Pause time between items:', 'presscoders' ); ?></label>
			<input id="<?php echo $this->get_field_id( 'pause_time' ); ?>" name="<?php echo $this->get_field_name( 'pause_time' ); ?>" type="text" value="<?php echo $pause_time; ?>" size="4" />
		</p>
	<?php
	}
