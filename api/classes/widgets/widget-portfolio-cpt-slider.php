...
class pc_portfolio_cpt_slider_widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'pc_portfolio_cpt_slider_widget_' . PC_THEME_NAME_SLUG, __( 'Portfolio Slider', 'presscoders' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<div style="margin-bottom:2px;">
			<label for="<?php echo $this->get_field_id( 'slider_groups' ); ?>"><?php _e( 'Choose portfolio group:', 'presscoders' ); ?></label>
		</div>
		<?php
		$args = array(
		...
		<label><input type="checkbox" value="1" <?php checked( $randomize, '1' ); ?> name="<?php echo $this->get_field_name( 'randomize' ); ?>" />&nbsp;<?php _e( 'Random slide order', 'presscoders' ) ?>
		</label><br />
		<label><input type="checkbox" value="1" <?php checked( $autoplay, '1' ); ?> name="<?php echo $this->get_field_name( 'autoplay' ); ?>" />&nbsp;<?php _e( 'Autoplay slides', 'presscoders' ) ?>
		</label><br />
		<label><input type="checkbox" value="1" <?php checked( $smooth_height, '1' ); ?> name="<?php echo $this->get_field_name( 'smooth_height' ); ?>" />&nbsp;<?php _e( 'Slide Smooth Height', 'presscoders' ) ?>
		</label><br />
		<label><input type="checkbox" value="1" <?php checked( $fi_post_link, '1' ); ?> name="<?php echo $this->get_field_name( 'fi_post_link' ); ?>" />&nbsp;<?php _e( 'Link featured image to post url', 'presscoders' ) ?>
		</label><br />
		...
		<label style="display:none;"><input type="checkbox" value="1" <?php checked( $animation_loop, '1' ); ?> name="<?php echo $this->get_field_name( 'animation_loop' ); ?>" />&nbsp;<?php _e( 'Loop Slides', 'presscoders' ) ?>
		</label>

		<p>
		<div style="margin-bottom:2px;"><?php _e( 'Transition settings', 'presscoders' ); ?>:</div>
		<?php _e( 'Effect', 'presscoders' ); ?>:
						...
						<?php while ( $r->have_posts() ) : $r->the_post(); ?>
							...
							if ( $fi_post_link ) {
								...
								if ( empty( $portfolio_fi_url ) ) {
									...
									if ( $portfolio_fi_url != 'https://none' ) {
							...
							if ( empty( $portfolio_title_url ) ) {
								...
								if ( $portfolio_title_url != 'https://none' ) {
