...
class pc_recent_posts_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'pc_recent_posts_widget_' . PC_THEME_NAME_SLUG,
			__( 'Recent Posts Excerpt', 'presscoders' ),
			array(
				'classname' => 'pc_recent_posts_widget',
				'description' => __( 'An alternative way to display recent posts. The post title, excerpt, and read more link are shown.', 'presscoders' )
			)
		);
	}
			...
			<ul class="theme-rpw">
				<?php while ( $r->have_posts() ) : $r->the_post(); ?>
					<li>
						...
						<p class="read-more">
							<a href="<?php the_permalink() ?>" title="<?php echo esc_attr( get_the_title() ? get_the_title() : get_the_ID() ); ?>"><?php _e( 'Read more', 'presscoders' ); ?></a>
						</p>
					</li>
				<?php endwhile; ?>
			</ul>
