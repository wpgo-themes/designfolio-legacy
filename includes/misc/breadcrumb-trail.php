...
function breadcrumb_trail( $args = array() ) {
	...
	$defaults = array(
		'separator'  => '/',
		//'before' => '<span class="breadcrumb-title">' . __( 'Browse:', 'presscoders' ) . '</span>',
		'before'     => '',
		'after'      => false,
		'front_page' => true,
		'show_home'  => __( 'Home', 'presscoders' ),
		'echo'       => true
	);
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) ) {
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'presscoders' ) );
			} elseif ( get_query_var( 'minute' ) ) {
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'presscoders' ), get_the_time( __( 'i', 'presscoders' ) ) );
			} elseif ( get_query_var( 'hour' ) ) {
				$trail['trail_end'] = get_the_time( __( 'g a', 'presscoders' ) );
			}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[]            = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'presscoders' ) ) . '">' . get_the_time( __( 'Y', 'presscoders' ) ) . '</a>';
				$trail[]            = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'presscoders' ) ) . '">' . get_the_time( __( 'F', 'presscoders' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'd', 'presscoders' ) );
			} elseif ( get_query_var( 'w' ) ) {
				$trail[]            = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'presscoders' ) ) . '">' . get_the_time( __( 'Y', 'presscoders' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'presscoders' ), get_the_time( esc_attr__( 'W', 'presscoders' ) ) );
			} elseif ( is_month() ) {
				$trail[]            = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'presscoders' ) ) . '">' . get_the_time( __( 'Y', 'presscoders' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'presscoders' ) );
			} elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'presscoders' ) );
			}
		}
	} /* If viewing search results. */
	elseif ( is_search() ) {
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'presscoders' ), esc_attr( get_search_query() ) );
	} /* If viewing a 404 error page. */
	elseif ( is_404() ) {
		$trail['trail_end'] = __( '404 Not Found', 'presscoders' );
	}
...
function breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '' ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'presscoders' ), $post_id ) . '">' . get_the_time( __( 'Y', 'presscoders' ), $post_id ) . '</a>';
			} /* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag ) {
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'presscoders' ), $post_id ) . '">' . get_the_time( __( 'F', 'presscoders' ), $post_id ) . '</a>';
			} /* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag ) {
				$trail[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'presscoders' ), $post_id ) . '">' . get_the_time( __( 'd', 'presscoders' ), $post_id ) . '</a>';
