	...
	if ( ! empty( $pf_group_exists ) ) :
		...
		if ( $r->have_posts() ) :
			...
			<ul id="filters">
				...
				?>
				<li class="active">
					<a href="javascript:void(0)" class="all"><?php echo ucwords( mb_strtolower( $parent_name, 'UTF-8' ) ); ?></a>
				</li>

				<?php foreach ( $cats as $cat ) { ?>
					<li>
						<a href="javascript:void(0)" class="<?php echo $cat->slug; ?>"><?php echo ucwords( mb_strtolower( $cat->cat_name, 'UTF-8' ) ); ?></a>
					</li>
				<?php } ?>
					...
					<?php while ( $r->have_posts() ) : $r->the_post(); ?>
						...
						if ( $pf_groups && ! is_wp_error( $pf_groups ) ) :
							...
							foreach ( $pf_groups as $pf_group ) {
								$pf_group_list[] = mb_strtolower( $pf_group->slug );

								/* See if there is a parent Portfolio group to add that isn't the root. */
								if ( $pf_group->parent != 0 ) {
									$parent_pf_term     = get_term( $pf_group->parent, 'portfolio_group' );
									$parent_pf_slug     = mb_strtolower( $parent_pf_term->slug, 'UTF-8' );
									$pf_root_group_term = get_term( $pc_portfolio_group, 'portfolio_group' );
									$pf_root_group_slug = mb_strtolower( $pf_root_group_term->slug, 'UTF-8' );

									/* Only add parent PF group if it isn't the root PF group, or not already in the term list.
									 * Note: The parent PF group could already be in the term list if a PF item has been manually associated with
									 * the parent group.
									*/
									if ( ( $parent_pf_slug != $pf_root_group_slug ) && ! in_array( $parent_pf_slug, $pf_group_list ) ) {
										$pf_group_list[] = $parent_pf_slug;
									}
								}
							}
